...
def split_data_into_train_val_test(src_folder, train_folder, val_folder, test_folder, train_ratio=0.85, val_ratio=0.05,
    ...
    subfolders = [subfolder for subfolder in src_folder.iterdir() if subfolder.is_dir() and "seg" not in subfolder.name]
...
# 源文件夹路径
src_folder = Path("datasets/source/vomit_v4_singleScene_transparent")

# 训练、验证和测试文件夹的路径
targetPathParent=src_folder.parent.parent / src_folder.name
train_folder = targetPathParent / "train" #Path("datasets/vomit_v4_singleScene_transparent/train")
val_folder = targetPathParent / "val" 
test_folder =  targetPathParent / "test"

# 划分数据集
split_data_into_train_val_test(src_folder, train_folder, val_folder, test_folder,val_ratio=0.1,vomitNum=2263)